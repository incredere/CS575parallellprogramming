...
main( int argc, char *argv[ ] )
{
    ...
    size_t resultSize = NUM_WORK_GROUPS * sizeof(float);
    ...
    cl_mem dC = clCreateBuffer( context, CL_MEM_READ_WRITE, resultSize, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueWriteBuffer( cmdQueue, dC, CL_FALSE, 0, resultSize, hC, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, resultSize, hC, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	fprintf( stderr, "%d,%d,%.3lf,%d\n",
		NMB, LOCAL_SIZE, (double)NUM_ELEMENTS/(time1-time0)/1000000000., NUM_WORK_GROUPS );
